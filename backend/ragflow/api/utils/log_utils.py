...
def init_ragflow_logger(
    ...
    logger_name: str = "ragflow",  # Child Logger Name
) -> logging.Logger:
    global initialized_ragflow_logger
    if initialized_ragflow_logger:
        return
    initialized_ragflow_logger = True

    logger = logging.getLogger(logger_name)  # Use Child Logger
    logger.handlers.clear()  # Clear handlers of Child Logger
    log_path = os.path.abspath(os.path.join(get_project_base_directory(), "logs", f"{logfile_basename}.log"))

    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    ragflow_formatter = logging.Formatter(log_format)
    openwebui_formatter = logging.Formatter(
        "%(asctime)-23s | %(levelname)-8s | RAGFlow | %(name)s:%(funcName)s:%(lineno)d - %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )

    handler1 = RotatingFileHandler(log_path, maxBytes=10 * 1024 * 1024, backupCount=5)
    handler1.setFormatter(ragflow_formatter)
    logger.addHandler(handler1)  # Mount handler on Child Logger

    handler2 = logging.StreamHandler()
    handler2.setFormatter(openwebui_formatter)
    logger.addHandler(handler2)  # Mount handler on Child Logger

    logger.propagate = False  # Do not propagate to Root Logger Handler even if Child Logger Handler is empty

    logging.captureWarnings(True)  # ( Note. warnings → Root Logger )
    ...
    for pkg_name in ["peewee", "pdfminer"]:
        if pkg_name not in pkg_levels:
            pkg_levels[pkg_name] = logging.getLevelName(logging.WARNING)
    if "ragflow" not in pkg_levels:  # Use Child Logger － ragflow, not Root Logger
        pkg_levels["ragflow"] = logging.getLevelName(logging.INFO)
...
def log_exception(e, *args):
    ...
    ragflow_logger.exception(e)
    for a in args:
        if hasattr(a, "text"):
            ragflow_logger.error(a.text)
