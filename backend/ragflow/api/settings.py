...
def get_or_create_secret_key():
    ...
    import logging
    ragflow_logger = logging.getLogger("ragflow")  # RAGFlow logger

    new_key = secrets.token_hex(32)
    ragflow_logger.warning(f"SECURITY WARNING: Using auto-generated SECRET_KEY. Generated key: {new_key}")
    return new_key
...
def init_settings():
    ...
    if lower_case_doc_engine == "elasticsearch":
        docStoreConn = es_conn.ESConnection()  # rag.utils.es_conn.ESConnection()
    elif lower_case_doc_engine == "infinity":
        docStoreConn = infinity_conn.InfinityConnection()  # rag.utils.infinity_conn.InfinityConnection()
    elif lower_case_doc_engine == "opensearch":
        docStoreConn = opensearch_conn.OSConnection()  # rag.utils.opensearch_conn.OSConnection()
    else:
